...
async function handleMcpRequest(req: IncomingMessage, res: ServerResponse, _config: Config): Promise<void> {
...
async function handleSSERequest(_req: IncomingMessage, res: ServerResponse, _config: Config): Promise<void> {
...
async function createNewSession(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  const transport = new StreamableHTTPServerTransport({
    ...
    onsessioninitialized: (sessionId) => {
      ...
      console.log('New spamsense session created:', sessionId);
    },
  });
  ...
  transport.onclose = () => {
    if (transport.sessionId) {
      ...
      console.log('Spamsense session closed:', transport.sessionId);
    }
  };
...
function handleHealthCheck(res: ServerResponse): void {
  ...
  res.end(
    JSON.stringify({ status: 'healthy', timestamp: new Date().toISOString(), service: 'spamsense-mcp', version: '0.2.0' })
  );
}
...
function logServerStart(config: Config): void {
  ...
  console.log(`Spamsense MCP Server listening on ${displayUrl}`);
